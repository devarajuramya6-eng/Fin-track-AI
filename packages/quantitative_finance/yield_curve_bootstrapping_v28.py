"""
Quantitative Finance Module: Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping (Variant 28)
...
@dataclass
class QuantConfig_yield_curve_bootstrapping_28:
    model_id: str = 'yield_curve_bootstrapping_28'
...
def calculate_yield_curve_bootstrapping_metric_1(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #1 with analytical sensitivity adjustments."""
...
def calculate_yield_curve_bootstrapping_metric_2(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #2 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_3(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #3 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_4(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #4 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_5(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #5 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_6(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #6 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_7(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #7 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_8(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #8 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_9(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #9 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_10(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #10 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_11(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #11 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_12(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #12 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_13(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #13 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_14(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #14 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_15(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #15 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_16(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #16 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_17(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #17 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
...
def calculate_yield_curve_bootstrapping_metric_18(spot: float, strike: float, rate: float, vol: float, maturity: float, params: Optional[Dict[str, float]] = None) -> Dict[str, Any]:
    """Compute Nelson-Siegel-Svensson and Cubic Spline Yield Curve Bootstrapping metric #18 with analytical sensitivity adjustments."""
    if maturity <= 0.0 or vol <= 0.0 or spot <= 0.0 or strike <= 0.0:
        return {'value': 0.0, 'delta': 0.0, 'gamma': 0.0, 'vega': 0.0, 'theta': 0.0, 'rho': 0.0, 'converged': False}
